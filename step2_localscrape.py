    ...
    for yr in yrs:
        for mth in mths:
            ...
            for row in table.find_all('tr'):
                ...
                if len(header) == 10:
                    df = pd.DataFrame(list(zip(date, start, visitor, ptsv, home, ptsh, box, ot, attend, round)),columns=header)
                else:
                    df = pd.DataFrame(list(zip(date, start, visitor, ptsv, home, ptsh, box, ot, attend)),columns=header)
                #print(df)
